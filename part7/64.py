...
# 1. dev_data から先頭の特徴辞書を取り出し
feat_first = dev_data[0]["feature"]

# 2. DictVectorizer でベクトル化
X_first = vec.transform([feat_first])  # 1×語彙数 の行列

# 3. predict_proba で各クラスの確率を取得
probs = model.predict_proba(X_first)[0]  # shape=(n_classes,)

# 4. クラス順を取得
labels = model.classes_  # e.g. array(['0','1'], dtype='<U1')

# 5. 結果表示
for label, p in zip(labels, probs):
    print(f"P(y={label} | x) = {p:.4f}")
